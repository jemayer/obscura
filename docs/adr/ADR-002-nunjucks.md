...
comagen needs a templating engine to render HTML pages from structured data (photo metadata, gallery listings, blog posts, config values). The templates must support inheritance (base layout with child pages), includes (reusable partials like navigation and photo cards), loops, conditionals, and custom filters.
